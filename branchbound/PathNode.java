...
class PathNode implements Comparable<PathNode> {
    ...
    private ArrayList<VerticesPair> verticesPairs; // Used to calculate the lower bound, contains the pairs with the
                                                   // edges for each vertex
    private PathNode previousPathNode; // Pointer to the previous path node

    // Overridden compareTo method, in order to compare two paths first by their
    // lower bound in the priority queue used
    @Override
    ...
    PathNode(int vertexToVisit, int costToReachVertex, PathNode parentPathNode) {
        ...
        if (parentPathNode.getPreviousPathNode() != null)
            verticesPairs.get(parentPathNode.getCurrentVertexId()).replaceVertex(currentVertexId, costToReachVertex,
                    parentPathNode.getPreviousPathNode().getCurrentVertexId());
        else
            verticesPairs.get(parentPathNode.getCurrentVertexId()).replaceVertex(currentVertexId, costToReachVertex,
                    -1);
        verticesPairs.get(currentVertexId).replaceVertex(parentPathNode.getCurrentVertexId(), costToReachVertex,
                parentPathNode.getCurrentVertexId());
