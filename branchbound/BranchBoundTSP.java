...
class BranchBoundTSP {
    ...
    private ArrayList<VerticesPair> initializeClosestPairs() {
        ...
        for (int i = 0; i < n; i++) {
            ...
            int closestVertexA = -1;
            int closestVertexB = -1;
    ...
    // Finds the shortest path using the branch and bound method and returns an
    // integer representing the shortest's path cost
    ...
    // Method to find the shortest path using the branch and bound method, using the
    // total sum of
    // the two closest edges for each vertex and then dividing with 2
    private void branchAndBound() {
        while (priorityQueue.size() > 0) {
            ...
            // If no path with lower bound smaller than the current best exists, returning
            // with the current path found
            if (currentPathNode.getLb() >= shortestPathCost)
                return;

            // If last vertex is reached and the path taken is shorter than the current one
            // taken then
            // marking this path as the solution and save it's cost
            if (currentPathNode.getTotalVisitedVertices() == n && graph[0][currentPathNode.getCurrentVertexId()] > 0
                    && currentPathNode.getPathTotalCost()
                            + graph[0][currentPathNode.getCurrentVertexId()] < shortestPathCost) {
            ...
            for (int i = 0; i < n; i++) {
                if (!currentPathNode.getVisitedVertices()[i] && graph[i][currentPathNode.getCurrentVertexId()] > 0) {
                    PathNode pathNode = new PathNode(i, graph[i][currentPathNode.getCurrentVertexId()],
                            currentPathNode);
    ...
    // Returns a Stack of integers representing the path of the best shortest route
    // to take
